...
lazygitではアイコンを使用しています。

MacOSの場合:~/Library/Application\ Support/lazygit/config.yml

アイコンの設定は`~/Library/Application\ Support/lazygit/config.yml`の`gui`の`showIcons`を`true`に設定することで使用可能です。

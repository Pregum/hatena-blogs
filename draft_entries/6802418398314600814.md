...
##### git checkout . | ステージングしていないファイルの削除

Filesパネル上で削除対象のファイルを選択して、`d`を入力することでステージングしていないファイルの削除が可能です。

削除前に一度ダイアログが出るので、`<enter>` を入力することで削除が実行されます。

// TODO: ここにステージングしていないファイルの削除の図を貼り付ける

##### git checkout {ブランチ名} | フランチの切り替え
...
Branchesパネル上で`n`を入力することで新規ブランチの作成が可能です。

// TODO: ここに新規ブランチの作成の図を貼り付ける
